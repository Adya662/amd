...
class CallTranscriptAnalyzer:
    ...
    def is_machine_answered(self, transcript: List[Dict], threshold: float = 3.0) -> Tuple[bool, float, List[str]]:
        """Determine if call was answered by machine"""
    ...
    def analyze_all_transcripts(self, call_ids: List[str]) -> Tuple[Dict[str, Any], pd.DataFrame]:
        ...
        for call_id, transcript in multi_utterance_calls:
            is_machine, score, matches = self.is_machine_answered(transcript)
            ...
            if is_machine:
                machine_calls.append(call_id)
            else:
                human_calls.append(call_id)
        
        self.stats.machine_answered = len(machine_calls)
        self.stats.human_answered = len(human_calls)
        
        print(f"Machine answered calls: {self.stats.machine_answered}")
        print(f"Human answered calls: {self.stats.human_answered}")
        ...
        summary = {
            ...
            'conversion_rates': {
                ...
                'machine_detection_rate': (self.stats.machine_answered / self.stats.after_single_utterance_filter * 100) if self.stats.after_single_utterance_filter > 0 else 0
            }
        }
    ...
    def create_data_funnel_visualization(self, summary: Dict[str, Any]):
        ...
        funnel_steps = [
            ...
            ("Machine Answered", stats.machine_answered, rates['machine_detection_rate']),
            ("Human Answered", stats.human_answered, 100 - rates['machine_detection_rate'])
        ]
        
        for i, (step_name, count, percentage) in enumerate(funnel_steps):
            if i < 3:  # Main funnel steps
                bar_length = int(percentage / 100 * 40)
                bar = "█" * bar_length + "░" * (40 - bar_length)
                print(f"{step_name:.<35} {count:>6} calls │{bar}│ {percentage:5.1f}%")
            else:  # Split for machine/human
                if i == 3:
                    print("    └── Split Analysis:")
                bar_length = int(percentage / 100 * 30)
                bar = "█" * bar_length + "░" * (30 - bar_length)
                print(f"        {step_name:.<25} {count:>6} calls │{bar}│ {percentage:5.1f}%")
...
def main():
    ...
    if not results_df.empty:
        for _, row in results_df.head().iterrows():
            ...
            print(f"  Machine: {'Yes' if row['is_machine'] else 'No'} (Score: {row['machine_score']:.1f})")
            print(f"  Utterances: {row['utterance_count']}")
            if row['matches']:
                print(f"  Matches: {', '.join(row['matches'][:3])}")  # Show first 3 matches
    ...
    # Summary statistics
    print(f"\nSUMMARY STATISTICS:")
    print(f"Machine Detection Accuracy Indicators:")
    print(f"  - Average machine score: {results_df[results_df['is_machine']]['machine_score'].mean():.2f}")
    print(f"  - Average human score: {results_df[~results_df['is_machine']]['machine_score'].mean():.2f}")
